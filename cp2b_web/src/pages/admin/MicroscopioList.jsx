...
const MicroscopioList = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      setError('Erro ao carregar artigos do Microscópio');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (slug) => {
    ...
    setDeleting(slug);
    try {
      await api.delete(`/microscopio/${slug}`);
  ...
  return (
    <Container>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>Microscópio de Ideias</h2>
        <Button as={Link} to="/admin/microscopio/new" variant="primary">
              ...
              <tr key={item.slug}>
                ...
                <td>
                  <Button
                    ...
                    size="sm"
                    className="me-2"
                    onClick={() => navigate(`/admin/microscopio/${item.slug}`)}
                  >
...
export default MicroscopioList;
