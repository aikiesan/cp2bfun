...
const MicroscopioEditor = () => {
  ...
  const fetchItem = async () => {
    try {
      ...
      setError('Erro ao carregar artigo do Microscópio');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await api.put(`/microscopio/${slug}`, payload);
      } else {
        await api.post('/microscopio', payload);
      }

      setSuccess(true);
      setTimeout(() => navigate('/admin/microscopio'), 1500);
    } catch (err) {
  ...
  return (
    <Container>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>{isEditing ? 'Editar Artigo do Microscópio' : 'Novo Artigo do Microscópio'}</h2>
        <Button variant="outline-secondary" onClick={() => navigate('/admin/microscopio')}>
              ...
              <Card.Body>
                <Form.Group className="mb-3">
                  ...
                  <Form.Text className="text-muted">
                    Ex: /microscopio/{formData.slug || 'meu-artigo'}
                  </Form.Text>
                </Form.Group>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    onChange={handleChange}
                    placeholder="Ex: Opiniao, Analise, Reflexao"
                  />
                </Form.Group>
...
export default MicroscopioEditor;
